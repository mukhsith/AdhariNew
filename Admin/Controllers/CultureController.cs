...
namespace Admin.Controllers
{
    public class CultureController : Controller
    {
        ...
        public async Task<IActionResult> SetCulture(string culture, string returnUrl)
        {
            try
            {
                //int.TryParse(Request.Cookies["CustomerId"], out int customerId);
                //if (customerId > 0)
                //{
                //    await _apiHelper.GetAsync<APIResponseModel<object>>("api/accountsmobile/switchlanguage?language=" + culture + "&customerId=" + customerId);
                //}
