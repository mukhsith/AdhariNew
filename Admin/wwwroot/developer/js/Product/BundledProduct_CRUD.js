...
loadDataFor = () => {
    ...
    ajaxGet('BundledProduct/ById?Id=' + parentId, cbGetSuccess);
};
cbGetSuccess = (data) => {
...
saveData = () => {
    ...
    ajaxPost("BundledProduct/AddEdit", submitData, cbPostSuccess, cbPostError);
...
prepareDatatable = () => {
    ...
    Table = $('.datatable-default-bundledProduct').DataTable({
        ...
        serverSide: false,
        buttons: [],
        language: { emptyTable: "No Bundled Products Added." },
        ...
        if (!alreadyExistsInTable(item)) {
            Table.row.add(
                [
                    ...
                    `<input type='number' class='form-control form-control-lg border-secondary' min=1
    ...
    //    $('.total-price').html("KWD " + (qty * price));
    //    $('.total-price').attr("data-price", (qty * price));
    //    $('.total-qty').html(qty);
    //}
...
//                    totalPrice += PriceCell;
//                } else if (cellIndex == 4) { //qty column 4 <td><input type=number min=1/></td>
//                    const QtyCell = parseInt($(cell).find("input[type=number]")[0].value);
//                    totalQty += QtyCell;
//                }
//                //for debug
//                //console.log(cell.textContent);
...
//    $('.datatable-default-bundledProduct').find('tfoot').show();
//    setTotalPrice(Table.rows().count());
//    setTotalQty(Table.rows().count());
//}

//id = productDetail primaryId
//parentId = productDeatil parent primary id (Product) table for refrence (it will repeat for each children records)
//productId = productDetail newly added product id from dropdown list item
//quantity = number of items
//price = automatically take from product table